...
int getsetid(SQLite::Database &db, const std::string &name) {
  SQLite::Statement query(db, "SELECT MAX(id) FROM sets WHERE name = ?1 AND "
                              "rnbo_compat_version = ?2 GROUP BY name");
  query.bind(1, name);
...
std::unordered_map<int, int> setInstanceToPatcherId(SQLite::Database &db,
                                                    int set_id) {
  std::unordered_map<int, int> values;
  SQLite::Statement query(db, "SELECT set_instance_index, patcher_id FROM "
                              "sets_patcher_instances WHERE set_id = ?1");
  query.bind(1, set_id);
...
void insertSetViewParams(SQLite::Database &db, int set_id, int set_view_id,
                         const std::vector<ViewParam> &params) {
  std::unordered_map<int, int> instanceindexpatcherid =
      setInstanceToPatcherId(db, set_id);

  {
    SQLite::Statement query(
        db,
        "INSERT OR IGNORE INTO sets_views_params (set_view_id, sort_order, "
        "set_instance_index, param_id, patcher_id) VALUES(?1, ?2, ?3, ?4, ?5)");
    for (auto i = 0; i < params.size(); i++) {
      ...
      if (it != instanceindexpatcherid.end()) {
        ...
        std::cerr
            << "failed to find patcher id while inserting set view param data"
            << std::endl;
      }
    }
  }
...
void writePatchersParams(SQLite::Database &db, const int patcher_id,
                         const fs::path &config_name) {
  auto configp =
      config::get<fs::path>(config::key::SourceCacheDir).get() / config_name;

  if (!fs::exists(configp)) {
    std::cerr << "config file does not exist at path: " << configp.string()
              << std::endl;
    return;
  }
  ...
  if (!config.contains("parameters")) {
    std::cerr << "config file does not contain parameters: " << configp.string()
              << std::endl;
    return;
  }

  SQLite::Statement query(db, "INSERT INTO patchers_params (patcher_id, "
                              "param_id, param_index) VALUES(?1, ?2, ?3)");
  ...
  for (auto p : config["parameters"]) {
    if (p.contains("type") && p.contains("index") && p.contains("paramId")) {
      ...
      std::cerr << "config param entry does not contain expected keys: "
                << configp.string() << std::endl;
      break;
    }
  }
}
...
DB::DB()
    : mDB(config::get<fs::path>(config::key::DBPath).get().string(),
  ...
  // create initial version entry, if it doesn't already exist
  {
    SQLite::Statement query(
        mDB,
        "INSERT OR IGNORE INTO migrations (id, rnbo_version) VALUES (1, ?)");
    query.bind(1, runner::rnbo_version);
    query.exec();
  }
  ...
  auto do_migration = [curversion, &lastmigration, &backupdone,
                       this](int version,
                             std::function<void(SQLite::Database &)> func,
    ...
    if (curversion < version) {
      ...
      if (!backupdone) {
        std::string backupname =
            "oscqueryrunner-dbversion-" + std::to_string(curversion);
        backup(backupname);
        backupdone = true;
      }
      ...
      auto incr = [this, &version]() {
        SQLite::Statement query(
            mDB, "INSERT INTO migrations (id, rnbo_version) VALUES (?, ?)");
        query.bind(1, version);
        query.bind(2, runner::rnbo_version);
        query.exec();
      };
  ...
  do_migration(3, [](SQLite::Database &db) {
    ...
    db.exec("CREATE INDEX patcher_name_version ON patchers(name, "
            "runner_rnbo_version)");
  });
...
PRAGMA foreign_keys=on;
  ...
  do_migration(
      9, [](SQLite::Database &db) { db.exec("DROP TABLE _presets_old"); });
...
(
    ...
    db.exec("CREATE INDEX set_preset_patcher_id_instance_index ON "
            "sets_presets(patcher_id, set_id, set_instance_index)");
  });
...
(
  ...
  do_migration(14, [](SQLite::Database &db) {
    db.exec(
        "ALTER TABLE sets_presets ADD COLUMN preset_name TEXT DEFAULT NULL");
  });
...
(
  ...
  do_migration(16, [](SQLite::Database &db) {
    // data migrations indicate that we've copied over the data we hope to for
    // the runner_rnbo_version specified the lack of this entry can be used to
    // compute the presence of migration data
    db.exec(R"(
...
(
    ...
    // get the data, we only care about the latest set of patchers (if there are
    // any)
    {
      std::unordered_map<int, std::string> idtoconfig;
      {
        SQLite::Statement query(
            db, "SELECT id, config_path FROM patchers WHERE "
                "runner_rnbo_version IN (SELECT runner_rnbo_version FROM "
                "patchers ORDER BY id DESC LIMIT 1)");
        while (query.executeStep()) {
...
(
...
)");
    {
      ...
      std::vector<std::tuple<int, std::string, int>> data;
      {
        SQLite::Statement query(db,
                                "SELECT id, params, set_id FROM sets_views");
        while (query.executeStep()) {
          int id = query.getColumn(0);
          const char *s = query.getColumn(1);
      ...
      for (auto &entry : data) {
        ...
        for (auto p : params) {
          ...
          if (details.size() != 2) {
            std::cerr << "param entry not in expected format: " << p
                      << std::endl;
            continue;
          }
          ...
          if (it == instanceindexpatcherid.end()) {
            std::cerr << "couldn't find patcher_id for set_instance_index: "
                      << set_instance_index << std::endl;
            continue;
          }
          int patcher_id = it->second;

          SQLite::Statement query(db,
                                  "SELECT param_id FROM patchers_params WHERE "
                                  "patcher_id = ?1 AND param_index = ?2");
          query.bind(1, patcher_id);
          query.bind(2, param_index);
          if (!query.executeStep()) {
            std::cerr << "failed to get param_id from patchers_params with "
                         "patcher_id="
                      << patcher_id << " and param_index=" << param_index
                      << std::endl;
            continue;
          }
  ...
  do_migration(18, [](SQLite::Database &db) {
    db.exec("ALTER TABLE sets_presets ADD COLUMN preset_index INTEGER NOT NULL "
            "DEFAULT 0");
    db.exec("ALTER TABLE presets ADD COLUMN preset_index INTEGER NOT NULL "
            "DEFAULT 0");

    db.exec("CREATE INDEX sets_presets_index_index ON sets_presets(set_id, "
            "preset_index)");
    db.exec("CREATE INDEX presets_index_index ON presets(patcher_id, "
            "preset_index)");
  ...
  // uuids
  do_migration(19, [](SQLite::Database &db) {
    boost::uuids::string_generator parse;
    boost::uuids::uuid ns =
        parse("dfc97c68-1e76-4199-9f6d-c82d6138e398"); // UUID generated by xnor
                                                       // on march 26 2026
    boost::uuids::name_generator_sha1 gen(ns);
    ...
    auto updatetable = [&gen, &db](std::string tablename) {
      ...
      {
        SQLite::Statement query(db, "SELECT id, name, created_at FROM " +
                                        tablename);
        while (query.executeStep()) {
          int id = query.getColumn(0);
          std::string name = getStringColumn(query, 1);
          std::string created_at = getStringColumn(query, 2);

          std::string hash = tablename + ":" + name + ":" + created_at;
          boost::uuids::uuid result = gen(hash);
          iduuid.insert({id, boost::uuids::to_string(result)});
        }
      }

      {
        // prepared
        SQLite::Statement query(db, "UPDATE " + tablename +
                                        " SET uuid=?2 WHERE id=?1");
        for (auto &kv : iduuid) {
          query.bind(1, kv.first);
          query.bind(2, kv.second);
          query.exec();
          query.reset();
        }
      }
    };
    updatetable("patchers");
    updatetable("sets");
  });
...
std::string DB::backup(std::string backupname) {
  ...
  auto backuppath =
      config::get<fs::path>(config::key::BackupDir).get() / backupname;

  {
    std::lock_guard<std::mutex> guard(mMutex);
    SQLite::Database backupDB(backuppath.string(),
                              SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
    SQLite::Backup backup(backupDB, mDB);
    try {
      backup.executeStep();
    } catch (...) {
      std::cerr << "failed to backup db to: " << backuppath.string()
                << std::endl;
      return "";
    }
  }
...
void DB::rnboVersions(std::function<void(const std::string &)> f) {
  ...
  SQLite::Statement query(
      mDB,
      "SELECT DISTINCT(runner_rnbo_version) FROM patchers ORDER BY id DESC");
  while (query.executeStep()) {
...
boost::optional<std::string> DB::migrationDataAvailable() {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(
      mDB, "SELECT DISTINCT(rnbo_compat_version) FROM patchers WHERE "
           "rnbo_compat_version != ?1 AND rnbo_compat_version NOT IN (SELECT "
           "data_rnbo_version FROM data_migrations) ORDER BY id DESC LIMIT 1");
  query.bind(1, runner::rnbo_compat_version);
...
void DB::markDataMigrated() {
  ...
  SQLite::Statement query(
      mDB, "INSERT OR IGNORE INTO data_migrations (data_rnbo_version, "
           "runner_rnbo_version) SELECT DISTINCT(rnbo_compat_version), ?1 FROM "
           "patchers WHERE rnbo_compat_version != ?1 ORDER BY id DESC");
  query.bind(1, runner::rnbo_compat_version);
  query.exec();
}

bool DB::patcherLatestExistsWithUUID(const std::string &name,
                                     const std::string &uuid) {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(
      mDB,
      "SELECT COUNT(*) FROM patchers WHERE uuid = ?3 AND id IN (SELECT MAX(id) "
      "FROM patchers WHERE name = ?1 AND rnbo_compat_version = ?2)");
  query.bind(1, name);
  query.bind(2, runner::rnbo_compat_version);
  query.bind(3, uuid);
  if (query.executeStep()) {
...
void DB::patcherStore(const std::string &name, const fs::path &so_name,
                      const fs::path &config_name,
                      const fs::path &rnbo_patch_name,
                      const std::string &max_rnbo_version, bool migrate_presets,
                      int audio_inputs, int audio_outputs, int midi_inputs,
                      int midi_outputs, std::string uuid,
  ...
  {
    SQLite::Statement query(mDB, "SELECT MAX(id) FROM patchers WHERE name = ?1 "
                                 "AND rnbo_compat_version = ?2");
    query.bind(1, name);
    query.bind(2, runner::rnbo_compat_version);
    if (query.executeStep()) {
  ...
  if (runner_rnbo_version.size() == 0) {
    runner_rnbo_version = runner::rnbo_version;
  }
  ...
  if (old_id) {
    ...
    {
      SQLite::Statement query(
          mDB, "UPDATE sets_presets SET patcher_id = ?2 WHERE patcher_id = ?1");
      query.bind(1, old_id);
      query.bind(2, new_id);
      query.exec();
    }
    {
      SQLite::Statement query(mDB, "UPDATE sets_patcher_instances SET "
                                   "patcher_id = ?2 WHERE patcher_id = ?1");
      query.bind(1, old_id);
      query.bind(2, new_id);
      query.exec();
    }
    ...
    // update patcher_id for sets_views_params
    {
      SQLite::Statement query(
          mDB,
          "UPDATE sets_views_params SET patcher_id = ?2 WHERE patcher_id = ?1");
      query.bind(1, old_id);
      query.bind(2, new_id);
      query.exec();
    }
    // remove params from sets_views_params that no longer exist
    {
      SQLite::Statement query(mDB,
                              "DELETE FROM sets_views_params WHERE patcher_id "
                              "= ?1 AND param_id NOT IN (SELECT param_id FROM "
                              "patchers_params WHERE patcher_id = ?1)");
      query.bind(1, new_id);
      query.exec();
    }

    // TODO should any containing sets get new UUIDs because the patcher has
    // changed?
...
bool DB::patcherGetLatest(const std::string &name, fs::path &so_name,
                          fs::path &config_name, fs::path &rnbo_patch_name,
                          std::string &created_at, std::string &uuid,
                          std::string &compat_version,
                          std::string &patcher_rnbo_version,
                          std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;

  std::lock_guard<std::mutex> guard(mMutex);

  SQLite::Statement query(
      mDB,
      "SELECT so_path, config_path, rnbo_patch_name, created_at, uuid, "
      "rnbo_compat_version, runner_rnbo_version FROM patchers WHERE name = ?1 "
      "AND rnbo_compat_version = ?2 ORDER BY created_at DESC LIMIT 1");
  query.bind(1, name);
...
void DB::patcherDestroy(
    const std::string &name,
    std::function<void(boost::filesystem::path &so_name,
  ...
  {
    SQLite::Statement query(
        mDB, "SELECT so_path, config_path FROM patchers WHERE name = ?1 AND "
             "rnbo_compat_version = ?2 ORDER BY created_at DESC");
    query.bind(1, name);
    query.bind(2, runner::rnbo_compat_version);
    while (query.executeStep()) {
      const char *s = query.getColumn(0);
  ...
  }
  {
    SQLite::Statement query(
        mDB,
        "DELETE FROM patchers WHERE name = ?1 AND rnbo_compat_version = ?2");
    query.bind(1, name);
    query.bind(2, runner::rnbo_compat_version);
    query.executeStep();
  }
...
void DB::patcherRename(const std::string &name, std::string &newName) {
  ...
  SQLite::Statement query(mDB, "UPDATE patchers SET name = ?3 WHERE name = ?1 "
                               "AND rnbo_compat_version = ?2");
  query.bind(1, name);
  query.bind(2, runner::rnbo_compat_version);
...
void DB::patchers(std::function<void(const std::string &, int, int, int, int,
  ...
  while (query.executeStep()) {
    ...
    func(name, audio_inputs, audio_outputs, midi_inputs, midi_outputs,
         created_at, uuid, rnbo_version, compat_version);
  }
}

void DB::presets(const std::string &patchername,
                 std::function<void(const std::string &, bool, int)> f,
                 std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
std::vector<int> DB::presetIndexes(const std::string &patchername,
                                   std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
boost::optional<std::tuple<std::string, std::string, int>>
DB::preset(const std::string &patchername, const std::string &presetName,
           std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
boost::optional<std::tuple<std::string, std::string, int>>
DB::preset(const std::string &patchername, unsigned int index) {
  std::lock_guard<std::mutex> guard(mMutex);
  ...
  query.bind(1, index);
  query.bind(2, patchername);
  query.bind(3, runner::rnbo_compat_version);
  if (query.executeStep()) {
    const char *s = query.getColumn(0);
    std::string content(s);
...
void DB::presetSave(const std::string &patchername, std::string presetName,
                    const std::string &preset, int index) {
  std::lock_guard<std::mutex> guard(mMutex);
  ...
  {
    // XXX make sure to update patcherStore preset migration with any changes to
    ...
    query.bind(4, runner::rnbo_compat_version);
    query.bind(5, index);
    query.exec();
  }
}

void DB::presetSetInitial(const std::string &patchername,
...
void DB::presetRename(const std::string &patchername,
...
void DB::presetReindex(const std::string &patchername, const std::string &name,
                       int index) {
  if (index < 0) {
    return;
  }
...
void DB::presetDestroy(const std::string &patchername,
                       const std::string &presetName) {
  std::lock_guard<std::mutex> guard(mMutex);
...
std::vector<std::string> DB::setPresets(const std::string &setname,
                                        std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
std::vector<int> DB::setPresetIndexes(const std::string &setName,
                                      std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
DB::setPresetNameByIndex(const std::string &setName, unsigned int index,
                         std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
boost::optional<int> DB::setPresetIndexByName(const std::string &setName,
                                              const std::string &presetName,
                                              std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
int DB::setPresetIndexNext(const std::string &setName,
                           std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
void DB::setPresets(
    const std::string &setName, const std::string &presetName,
    std::function<void(const std::string &patcherName,
                       unsigned int instanceIndex, const std::string &content,
                       const std::string &patcherPresetName, int presetIndex)>
        func,
    std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
DB::setPreset(const std::string &patchername, const std::string &presetName,
              const std::string &setName, unsigned int instanceIndex,
              std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
  std::lock_guard<std::mutex> guard(mMutex);
...
void DB::setPresetSave(const std::string &patchername,
                       const std::string &presetName,
                       const std::string &setName, unsigned int instanceIndex,
                       const std::string &content,
                       std::string patcherPresetName, int presetIndex) {
  std::lock_guard<std::mutex> guard(mMutex);
...
void DB::setPresetRename(const std::string &setName, const std::string &oldName,
                         const std::string &newName, std::string rnbo_version) {
  if (oldName == newName) {
    return;
  }
  // BEFORE MUTEX
...
void DB::setPresetReindex(const std::string &setName, const std::string &name,
                          int presetindex, std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
  std::lock_guard<std::mutex> guard(mMutex);
...
void DB::setPresetDestroy(const std::string &setName,
                          const std::string &presetName,
                          std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
void DB::setPresetDestroyAll(const std::string &setName,
                             std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
  std::lock_guard<std::mutex> guard(mMutex);
...
void DB::setSave(const std::string &name, const SetInfo &info) {
  std::lock_guard<std::mutex> guard(mMutex);
  ...
  {
    SQLite::Statement query(
        mDB, "SELECT id FROM sets WHERE name = ?1 AND rnbo_compat_version = ?2 "
  ...
  if (id == 0) {
    ...
    {
      SQLite::Statement query(mDB,
                              "DELETE FROM sets_connections WHERE set_id = ?1");
      query.bind(1, id);
      query.exec();
    }
    {
      SQLite::Statement query(
          mDB, "DELETE FROM sets_patcher_instances WHERE set_id = ?1");
      query.bind(1, id);
      query.exec();
    }
  }
...
boost::optional<SetInfo> DB::setGet(const std::string &name,
                                    std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
  ...
  // get instances
  {
    ...
    while (query.executeStep()) {
      ...
      // construct an instance name, may have changed if the patcher has been
      // renamed
      instanceName.insert({index, name + "-" + std::to_string(index)});

      info.instances.push_back(
          SetInstanceInfo(name, static_cast<unsigned int>(index), config));
    }
  }
...
bool DB::setDestroy(const std::string &name) {
  // presets get deleted because of on delete cascade
  {
    std::lock_guard<std::mutex> guard(mMutex);
    SQLite::Statement query(
        mDB, "DELETE FROM sets WHERE name=?1 AND rnbo_compat_version=?2");
    query.bind(1, name);
    query.bind(2, runner::rnbo_compat_version);
    return query.exec() > 0;
  }
...
bool DB::setInitial(const std::string &name) {
  ...
  {
    std::lock_guard<std::mutex> guard(mMutex); // setNameInitial also grabs
                                               // mutex
    SQLite::Statement query(mDB,
                            "UPDATE sets SET initial = CASE WHEN name == ?1 "
                            "THEN 1 ELSE 0 END WHERE rnbo_compat_version=?2");
    query.bind(1, name);
    query.bind(2, runner::rnbo_compat_version);
    query.exec();
  }
  if (name.size() > 0)
    return setNameInitial(runner::rnbo_compat_version).has_value();
  return true;
}
...
bool DB::setRename(const std::string &oldName, const std::string &newName) {
  ...
  {
    SQLite::Statement query(mDB, "UPDATE OR IGNORE sets SET name=?3 WHERE "
                                 "name=?1 AND rnbo_compat_version=?2");
...
bool DB::setMatchesConnections(
  ...
  {
    SQLite::Statement query(
        mDB, "SELECT sink_name, sink_port_name FROM sets_connections WHERE "
             "source_name=?1 AND source_port_name=?2 AND set_id=?3");
...
boost::optional<std::string> DB::setNameInitial(std::string rnbo_version) {
  ...
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(mDB,
                          "SELECT name FROM sets WHERE rnbo_compat_version = "
                          "?1 AND initial = 1 ORDER BY name ASC LIMIT 1");
  query.bind(1, rnbo_version);
...
boost::optional<std::string> DB::setNameByIndex(unsigned int index,
                                                std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
  ...
  SQLite::Statement query(mDB,
                          "SELECT name FROM sets WHERE rnbo_compat_version = "
                          "?1 ORDER BY name ASC LIMIT 1 OFFSET ?2");
  query.bind(1, rnbo_version);
...
void DB::sets(
    std::function<void(const std::string &name, const std::string &created,
                       bool initial, const std::string &uuid)>
        func,
    std::string rnbo_version) {
  if (rnbo_version.size() == 0)
    rnbo_version = runner::rnbo_compat_version;
...
boost::optional<std::tuple<std::string, std::vector<std::string>, int>>
...
int DB::setViewCreate(const std::string &setname, const std::string &viewname,
                      const std::vector<ViewParam> params, int viewIndex) {
  int set_id = 0;
  {
    std::lock_guard<std::mutex> guard(mMutex);
  ...
  if (viewIndex < 0) {
    std::lock_guard<std::mutex> guard(mMutex);
    SQLite::Statement query(
        mDB, "SELECT MAX(view_index) + 1 FROM sets_views WHERE set_id = ?1");
    query.bind(1, set_id);
...
void DB::setViewDestroy(const std::string &setname, int viewIndex) {
  std::lock_guard<std::mutex> guard(mMutex);
  if (viewIndex < 0) {
    SQLite::Statement query(mDB, R"(
...
void DB::setViewUpdateParams(const std::string &setname, int viewIndex,
                             const std::vector<ViewParam> params) {
  std::lock_guard<std::mutex> guard(mMutex);
  ...
  SQLite::Transaction transaction(mDB);
  SQLite::Statement query(
      mDB, "DELETE FROM sets_views_params WHERE set_view_id = ?1");
  query.bind(1, set_view_id);
  query.exec();
...
void DB::setViewUpdateName(const std::string &setname, int viewIndex,
                           const std::string &name) {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(mDB, R"(
		UPDATE sets_views
...
bool DB::setViewsUpdateSortOrder(const std::string &setname,
                                 std::vector<int> &indexes) {
  std::lock_guard<std::mutex> guard(mMutex);
  ...
  if (setid > 0) {
    ...
    {
      SQLite::Statement query(mDB, "SELECT view_index FROM sets_views WHERE "
                                   "set_id = ?1 ORDER BY sort_order");
      query.bind(1, setid);
...
void DB::setViewsCopy(const std::string &srcSetName,
    ...
    }
    {
      SQLite::Statement query(mDB, R"(
				INSERT OR IGNORE INTO sets_views_params
					...
					SELECT sv_dst.id, svp.sort_order, svp.set_instance_index, svp.param_id, svp.patcher_id
					FROM sets_views_params AS svp
					JOIN sets_views AS sv_src ON svp.set_view_id = sv_src.id
					JOIN sets_views AS sv_dst ON sv_src.name = sv_dst.name AND sv_src.view_index = sv_dst.view_index AND sv_src.sort_order = sv_dst.sort_order
					WHERE sv_src.set_id = ?1 AND sv_dst.set_id = ?2
				)");
      query.bind(1, srcid);
      query.bind(2, dstid);
      query.exec();
...
bool DB::listenerExists(const std::string &ip, uint16_t port) {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(
      mDB, "SELECT COUNT(*) from listeners WHERE ip = ?1 AND port = ?2");
  query.bind(1, ip);
  query.bind(2, port);
  if (query.executeStep()) {
    int cnt = query.getColumn(0);
...
bool DB::listenersAdd(const std::string &ip, uint16_t port) {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(
      mDB, "INSERT OR IGNORE INTO listeners (ip, port) VALUES (?1, ?2)");
  query.bind(1, ip);
  query.bind(2, port);
  query.exec();
...
bool DB::listenersDel(const std::string &ip, uint16_t port) {
  std::lock_guard<std::mutex> guard(mMutex);
  SQLite::Statement query(mDB,
                          "DELETE FROM listeners where ip = ?1 AND port = ?2");
  query.bind(1, ip);
  query.bind(2, port);
  query.exec();
  return mDB.getChanges() != 0;
...
void DB::listeners(
    std::function<void(const std::string &ip, uint16_t port)> func) {
  std::lock_guard<std::mutex> guard(mMutex);
...
SetConnectionInfo SetConnectionInfo::fromJson(const RNBO::Json &json) {
  ...
  SetConnectionInfo info(json["source_name"].get<std::string>(),
...
RNBO::Json SetInstanceInfo::toJson() {
  return {{"patcher_name", patcher_name},
...
SetInfo SetInfo::fromJson(const RNBO::Json &json) {
  ...
  if (json.contains("set_info_version")) {
    if (json["set_info_version"].get<int>() == 2) {
      ...
      if (json.contains("runner_rnbo_version") &&
          json["runner_rnbo_version"].is_string()) {
        info.runner_rnbo_version =
            json["runner_rnbo_version"].get<std::string>();
      }
    } else {
      // invalid
    ...
    if (json.contains("instances") && json["instances"].is_array()) {
      for (auto &i : json["instances"]) {
        ...
        SetInstanceInfo inst(i["name"].get<std::string>(),
                             static_cast<unsigned int>(index), conf);
        instanceNameToIndex.insert(
            {inst.patcher_name + "-" + std::to_string(index), index});
        info.instances.push_back(inst);
      }
    }
    if (json.contains("connections") && json["connections"].is_object()) {
      auto cleanup_name_info =
      ...
      for (auto &kv : json["connections"].items()) {
        ...
        if (entry.is_object() && entry.contains("output") &&
            entry["output"].is_boolean() && entry["output"].get<bool>()) {
          if (entry.contains("connections") &&
              entry["connections"].is_array()) {
            for (auto c : entry["connections"]) {
              if (c.is_string()) {
                name = c.get<std::string>();
                std::vector<std::string> sink_info = cleanup_name_info(name);
                std::string sink_name = sink_info[0],
                            sink_port_name = sink_info[1];
                ...
                SetConnectionInfo conn(source_name, source_port_name, sink_name,
                                       sink_port_name);
