...
(
	...
	do_migration(19, [](SQLite::Database& db) {
		...
		boost::uuids::uuid ns = parse("dfc97c68-1e76-4199-9f6d-c82d6138e398"); //UUID generated by xnor on march 26 2026
		...
		auto updatetable = [&gen, &db](std::string tablename) {
			...
			{
				SQLite::Statement query(db, "SELECT id, name, created_at FROM " + tablename);
				while (query.executeStep()) {
					int id = query.getColumn(0);
			...
			{
				//prepared
				SQLite::Statement query(db, "UPDATE " + tablename + " SET uuid=?2 WHERE id=?1");
				for (auto& kv: iduuid) {
...
void DB::patcherStore(
	...
	if (old_id) {
		...
		//TODO should any containing sets get new UUIDs because the patcher has changed?
...
void DB::setSave(
	...
	if (id == 0) {
		...
		{
			SQLite::Statement query(mDB, "UPDATE sets SET meta = ?1, uuid = ?3 WHERE id = ?2");
