...
namespace config {
	namespace key {
		...
		const static std::string RecordingDir = "recording_dir"; //the directory where data (audio) files can be read from or written to
		...
		const static std::string PackageDir = "package_dir"; //where do we store import/exported package data (sets, patchers)
		...
		const static std::string InstanceDefaultSendOSCMIDI = "instance_default_send_osc_midi"; //should instances send OSC MIDI by default.. this can slow things down if you're sending a lot of MIDI
