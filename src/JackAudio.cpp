	...
	mBuilder([this](ossia::net::node_base * root) {
			...
			card->add_callback([this](const ossia::value& val) {
				if (val.get_type() == ossia::val_type::STRING) {
					...
					jconfig_set(mCardName, "card_name");
					//set extra args associated with this soundcard if we have them and there haven't been some others set
					if (!mExtraArgsDirty && mCardName.size() && mExtraArgsParam) {
						mExtraArgsParam->push_value(jextraconfig_get<std::string>(mCardName).value_or(""));
						mExtraArgsDirty = false;
					}
				}
			});
			...
			}
			{
				...
				p->add_callback([this](const ossia::value& val) {
					if (val.get_type() == ossia::val_type::STRING) {
						...
						mExtraArgsDirty = mExtraArgs.size() != 0;
...
bool ProcessAudioJack::createServer() {
	...
	mExtraArgsDirty = false; //once we try them, they're no longer dirty
	...
	auto create = [this]() -> bool {
		...
		//store extra args with card if we have it
		if (cardname.size()) {
			//if we either have extra content OR there is stored config content and we need to empty it, store
			if (extra.size() || jextraconfig_get<std::string>(cardname)) {
				jextraconfig_set(extra, cardname);
			}
		}
